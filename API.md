...
| POST | /api/places/:id/geocode | | the row with `geocoded_by openmeteo` (the title with only the region after the last comma of `herCity`, "Deering Oaks Park, Maine", then the bare title; 30 results each, the first within 30 km kept; 404 `no_match` when nothing within 30 km of her city; 502 `provider_failed`; the stub answers Stubtown, which is not near Portland) |
| POST | /api/places/:id/picture | `{ remake?: boolean }` | the row with `picture: true` (held open like a photo; 402 under the caps or `price_unknown` at 0 on a paid image provider, 404, 409 `already_generated` without `remake`, 409 `in_progress` while another call for the same place holds its claim (taken before the paid call, released after; a claim older than three minutes is taken over), 502, 503); one `model_runs` row of kind `place` at `placeCostUsd`; no `visual_assets` row, no approval flow, never the outfit rule or the character export |
...
| GET | /api/spotify/callback | `?code&state` (or `?error`) | 302 to `/model#spotify`; every refusal is a 302 to `/model?spotify=<code>#spotify` instead of an error body (a top-level navigation lands on the Model page, never raw JSON): the code is the error's detail or code, lowercased to `[a-z0-9_]`, 40 at most (`access_denied` and the like on `error`; `state_mismatch` on a state mismatch, nothing stored; `me_forbidden`, `no_refresh_token` or `provider_failed` on a token, profile or playlist failure). The playlist is made with `POST /v1/me/playlists` and songs added with `POST /v1/playlists/{id}/items` (Spotify's February 2026 Development Mode paths). Stores the tokens in `spotify_auth`, creates the private playlist once when `spotifyPlaylistId` is empty, sets `spotifyEnabled` true, audits `spotify.connected` with ids only |
| GET | /api/spotify/token | | `{ accessToken, expiresAt, premium, deviceName: "Avelie", player }` for the Web Playback SDK (refreshed under five minutes; `premium` true, false, or null when `/v1/me` carries no `product` (the Development Mode answer since February 2026) or the read failed; a read answer is cached a day, a failure never; the page refuses the SDK only on false); 403 `not_connected`. Never audited, never logged; the refresh token never leaves the Worker |
