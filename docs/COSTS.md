...
When his reference photos ride on a turn (SPEC_V3 section JJ: his first turn of a conversation, a turn that mentions his looks, every Together turn while `hisFaceInTogether` is on, every `hisFaceApartEvery` replies in Apart mode), each photo is sent as an image block on the final user turn, uncached: a 2000x2000 photo is about 1,600 input tokens on Anthropic, so at the default three photos a turn carries about 4,800 extra input tokens, which at claude-opus-5-5's input price is under a cent a turn and at claude-opus-5's 5.00 per million about 2.4 cents. The cached constitution prefix is untouched (the photos are on the message, not the system prompt). A Together conversation is where it adds up; lower `hisFaceMax` to 1 or turn the Together switch off on the Model page to trim it. A database without migration 0007 (no `conversations.his_face_seq`) shows no Apart cadence at all rather than the photos on every Apart turn (fix 2: a failed cadence read counts as just shown), so a stale schema can only cost less, never about 4,800 tokens a turn more. The describe call (`POST /api/him/describe`) is one small paid call on the story performer, estimated with the photos counted at that rate and metered as kind `describe`. Uploads, the words and the State page are free.

## v4: the phone, the places, the call face, Spotify, her clips, her voice

- The "listening to" line (Phone page and the chat's slide-in): one small text call a day on her story performer (`maxTokens` 120, effort low, cached per her local day in `panel_cache`; a refusal is cached too), estimated and gated by `assertBudget` and recorded as a run of kind `listening`. About a cent a day on claude-opus-5-5, under three cents on claude-opus-5. `listeningLineEnabled` false costs nothing.
- A place picture: `placeCostUsd` (0.08 on Runway `gen4_image` at the app's size; 0.04 to 0.06 on the OpenAI fallback at 1024x1536), once per place unless he presses Remake, metered as kind `place`, charged like a photo. It must be above 0 on a paid image provider (the Model page refuses 0 unless the provider is keyless; a picture at 0 on a paid provider is refused `402 price_unknown`). The map, the pins and the geocode are free (Open-Meteo).
- The call face: three clips (idle, listening, talking) at `videoCostUsd` x 5 / 5 each, charged at start like any clip: 0.75 USD for the set at the shipped 0.25 (gen4_turbo, five credits a second, one cent a credit). Nothing per call afterwards (the clips are served from R2). Remaking a kind is one more clip.
- A picture of the two of you: nothing beyond the photo (one more reference rides in the same request at the same flat price).
- Spotify: zero on the meter. The connect, the search and the add are free API calls on his account; the playlist is his; the player streams through his Premium account (Spotify's price, not this app's).
- Her clips from the chat (A3): `videoCostUsd` per five seconds (0.25 at the shipped price for a five-second clip; 0.50 at ten), charged at start through the caps; a clip she sends is her choice, at most one per message, so a talkative day is a few of them, not one per reply. `videoMarkerEnabled` false costs nothing.
- The delayed-reply notification, the album, the memory map and the phone panel's other lines are free (no model call).
- Her own voice (A2): voice notes on ElevenLabs at `elevenLabsTtsPricePer1kChars` (0.30 USD per 1,000 characters, list price to confirm on the ElevenLabs pricing page; a 200-character note is six cents) through `assertBudget`; calls on ElevenLabs at `elevenLabsCallPricePerMinute` (0.10 a minute, the setting's default, to confirm against his plan's Conversational AI minutes) per started minute against the caps, `callMaxMinutes` still applying. Both paths stay on OpenAI and Workers AI until the key and the ids exist.
