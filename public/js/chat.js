...
function renderMessage(m, flags, errorCode) {
  ...
  if (!his && !op) {
    ...
    const photo = renderPhoto(m, errorCode);
...
function renderPhoto(m, errorCode) {
...
async function requestPhoto(m) {
  ...
  try {
    ...
    if (e.status === 409) {
      ...
      replacePhoto({ ...m, image_status: "failed" }, e.code || "error");
    }
  } finally {
...
function replacePhoto(m, errorCode) {
