...
import type {
  ...
  PromptCallback, PromptState, ProviderName, Settings, TextProvider, TurnResponse, VisualAssetRow,
} from "./types";
...
export async function prepareTurn(
  ...
  const checkCtx: CheckContext = {
    ...
    channel: "story",
    // v3: the offered bank lines (exemplar_verbatim), the open asks (ask_nag), the shape window (shape_uniform).
    exemplars: (state.exemplars ?? []).map((l) => l.text).filter((t) => typeof t === "string" && t),
...
export async function commitReply(
  ...
  const buildCommit = async (): Promise<D1PreparedStatement[]> => {
    ...
    stmts.push(contextStmt(db, assistantRow.id, context, prepared.stateText));
