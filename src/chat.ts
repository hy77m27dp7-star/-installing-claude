...
import { generateCandidate, parsePhotoMarker } from "./images";
import { extractProposals } from "./proposals";
import { assertBudget, costMicro, estimateUsd } from "./budget";
import {
  dayKey, findByIdempotencyKey, getConversation, insertMessageStmt, insertModelRunStmt, newId, nextSeq, nowIso,
...
export async function runTurn(
  ...
  const assistantRow: MessageRow = {
    ...
    image_id: null,
    image_status: chosen.photo ? "pending" : null,
  };
  ...
  const response: TurnResponse = {
    ...
    imagePending: chosen.photo !== null,
    replayed: false,
  };

  // 10. after the response: the photo candidate and the proposal pass, both best effort
  const after: Promise<unknown>[] = [];
  if (chosen.photo) {
    after.push(
      generateCandidate(env, db, settings, { conversationId, messageId: assistantRow.id, description: chosen.photo, actor })
        .catch((e: unknown) => console.warn("photo candidate failed", errorClass(e))),
    );
  }
  ...
  if (after.length) ctx.waitUntil(Promise.allSettled(after));
